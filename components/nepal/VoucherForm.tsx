...
export default function NepalVoucherForm() {
  ...
  const submitWithRetry = async (
    ...
    while (attempt <= maxRetries) {
      try {
        const response = await fetch(`/tally/api/nepal/${type}`, {
          method: "POST",
...
//`${formatDate(syncMeta?.start_date)} → ${formatDate(syncMeta?.end_date)}`
      const pushedInvoiceRange = {
        start: formatDate(syncMeta?.start_date) || "",
        end: formatDate(syncMeta?.end_date) || "",
      };

      const duplicatePushes = selected
        .filter((v => {
          const entryDate = v.SaleEntryDate?.split("T")[0] || ""; 
          return (
            entryDate >= pushedInvoiceRange.start &&
            entryDate <= pushedInvoiceRange.end
          );
        }))
        .map((v) => v.InvoiceNo)
